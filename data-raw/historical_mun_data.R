...
historical_raw_mun_data <-
  bind_rows(historical_raw_mun_data_congress,
            historical_raw_mun_data_senate)

usethis::use_data(historical_raw_mun_data, overwrite = TRUE,
                  compress = "xz")
...
historical_raw_mun_data |>
  ...
  map(function(y) { map(y, function(x) {
    write_csv(x, file =
                glue("./data/csv/mun_data/raw_mun_data_{unique(x$type_elec)}_{year(unique(x$date_elec))}_{month(unique(x$date_elec))}.csv"))})})
