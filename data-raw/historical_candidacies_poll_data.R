...
historical_candidacies_poll_congress <-
  congress_elec |>
  rowwise() |>
...
historical_raw_candidacies_poll <-
  bind_rows(historical_candidacies_poll_congress,
            historical_candidacies_poll_senate)

# too much size
# usethis::use_data(historical_raw_candidacies_poll, overwrite = TRUE,
#                   compress = "xz")
...
historical_raw_candidacies_poll |>
  ...
  map(function(y) { map(y, function(x) {
    write_csv(x, file =
                glue("./data/csv/candidacies_pollstation/raw_candidacies_poll_{unique(x$type_elec)}_{year(unique(x$date_elec))}_{month(unique(x$date_elec))}.csv"))})})
