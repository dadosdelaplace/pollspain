...
#'
#' @export
import_survey_data <- function(type_elec = "congress", year = NULL, date = NULL,
                               verbose = TRUE) {
  ...
  survey_data <- pollspaindata::historical_surveys |>
    filter(id_elec %in% glue("{allowed_elections$cod_elec}-{allowed_elections$date}"))

  dict_parties <- global_dict_parties |>
  ...
  survey_data <-
    ...
    select(-contains(".rm")) |>
    relocate(id_candidacies_nat, name_candidacies_nat, .after = abbrev_candidacies) |>
    rename("estimated_porc_ballots" = "estimated_voting")
